...
- Базовый системный штат агентов включает `dev` и `reviewer` как обязательные роли review-контура: для всех `run:*`, кроме `run:abort`, pre-review обязателен перед финальным Owner review.
