...
- Идемпотентность trigger-обработки обязательна для всех stage-лейблов:
  - повторная доставка одного и того же webhook delivery/event не должна создавать второй run;
  - duplicate-delivery фиксируется как no-op в audit (`flow_events`) с диагностикой причины;
  - для doc-stage проверок (`run:prd`, `run:prd:revise`) это входит в обязательный e2e acceptance набор (Issue #132).
