...
- При постановке trigger-лейбла платформа сразу даёт обратную связь в issue:
  ...
  - идемпотентность обязательна: для одного `run_id` и текущей фазы допускается только один run-status комментарий (retry/update не создаёт дубликаты, а обновляет существующий comment id).
