...
related_issues: [1, 74, 90, 112, 152]
related_prs: []
approvals:
  required: ["Owner"]
  status: approved
...
| NFR-019 | Для каждого `run_id` и фазы сервисного статуса в GitHub Issue допускается не более одного активного run-status комментария платформы; повторные update/retry должны обновлять существующий комментарий, а не создавать дубликат. |
