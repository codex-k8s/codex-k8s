...
| S11 | `docs/delivery/sprints/s11/sprint_s11_telegram_user_interaction_adapter.md` | `docs/delivery/epics/s11/epic_s11.md` | in-review | Новый последовательный stream для Telegram-адаптера как первого внешнего channel path: Day1 intake (`#361`) зафиксировал зависимость от core Sprint S10, MVP scope notify/decision/callback/free-text, Context7 baseline по `telego` и handover issue `#444` для `run:vision`. |
