...
| S16 | `docs/delivery/sprints/s16/sprint_s16_mission_control_graph_workspace.md` | `docs/delivery/epics/s16/epic_s16.md` | in-review | Новый redesign stream для Mission Control graph workspace: Day1 intake (`#492`) зафиксировал полный product reset относительно текущего dashboard, issue `#480` как inventory foundation layer, hybrid truth matrix, filtered multi-root graph, Wave 1 nodes `discussion/work_item/run/pull_request`, typed metadata/watermarks/launch params и continuity rule `PR + follow-up issue`; Day2 vision (`#496`) закрепил mission/KPI/guardrails; Day3 PRD (`#510`) формализовал user stories/FR/AC/NFR; Day4 architecture (`#516`) закрепил ownership split; Day5 design (`#519`) выпустил graph-first transport/data/migration contract, а Day6 plan (`#537`) создал execution package и handover issues `#542..#547` для owner-managed `run:dev` waves. |
| S17 | `docs/delivery/sprints/s17/sprint_s17_unified_user_interaction_waits_and_owner_feedback_inbox.md` | `docs/delivery/epics/s17/epic_s17.md` | in-review | Новый cross-cutting stream для long-lived owner feedback loop: Day1 intake (`#541`) зафиксировал same live session как primary happy-path, snapshot-resume как recovery-only fallback, long wait target `>=24h`, delivery-before-wait lifecycle, Telegram pending inbox, staff-console fallback и persisted text/voice binding; создана continuity issue `#554` для `run:vision`. |
