...
2. Закрепить same-session continuation как primary happy-path: live pod / live `codex` session ждут пользователя, а snapshot-resume остаётся только failure-recovery механизмом.
