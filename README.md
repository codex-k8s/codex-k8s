...
- `run:debug` — отключает авто-cleanup по TTL для run namespace (manual cleanup mode).
...
Практический смысл:
- без `run:debug` namespace живёт по TTL policy; `run:debug` переключает его в manual cleanup mode.
...
3. Базово namespace сохраняется по TTL policy; при `run:debug` авто-cleanup отключается, и namespace удерживается до ручного удаления.
