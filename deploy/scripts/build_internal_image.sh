...
CODEXK8S_INTERNAL_REGISTRY_HOST="${CODEXK8S_INTERNAL_REGISTRY_SERVICE}.${CODEXK8S_STAGING_NAMESPACE}.svc.cluster.local:${CODEXK8S_INTERNAL_REGISTRY_PORT}"
...
if [ "$CODEXK8S_ENSURE_REGISTRY" = "true" ]; then
  ...
  kubectl -n "${CODEXK8S_STAGING_NAMESPACE}" rollout status "statefulset/${CODEXK8S_INTERNAL_REGISTRY_SERVICE}" --timeout="${CODEXK8S_REGISTRY_ROLLOUT_TIMEOUT}"
fi

kubectl -n "${CODEXK8S_STAGING_NAMESPACE}" create secret generic codex-k8s-git-token \
  --from-literal=token="${CODEXK8S_GITHUB_TOKEN}" \
