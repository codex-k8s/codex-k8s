...
printf %s "${CODEXK8S_GITHUB_TOKEN}" | gh auth login --with-token
...
gh secret set CODEXK8S_GITHUB_TOKEN -R "${CODEXK8S_GITHUB_REPO}" --body "${CODEXK8S_GITHUB_TOKEN}"
...
for legacy_secret in CODEXK8S_GITHUB_PAT CODEXK8S_GITHUB_USERNAME; do
  gh secret delete "${legacy_secret}" -R "${CODEXK8S_GITHUB_REPO}" >/dev/null 2>&1 || true
done
