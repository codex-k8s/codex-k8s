...
CODEXK8S_ENABLE_GITHUB_RUNNER="${CODEXK8S_ENABLE_GITHUB_RUNNER:-false}"
...
CODEXK8S_RUNNER_SCALE_SET_NAME="${CODEXK8S_RUNNER_SCALE_SET_NAME:-${CODEXK8S_STAGING_NAMESPACE}}"
CODEXK8S_RUNNER_IMAGE="${CODEXK8S_RUNNER_IMAGE:-ghcr.io/actions/actions-runner:latest}"
CODEXK8S_STAGING_RUNNER="${CODEXK8S_STAGING_RUNNER:-${CODEXK8S_RUNNER_SCALE_SET_NAME}}"
...
CODEXK8S_PROJECT_DB_LIFECYCLE_ALLOWED_ENVS="${CODEXK8S_PROJECT_DB_LIFECYCLE_ALLOWED_ENVS:-dev,staging,ai-staging,prod}"
...
ensure_repo_label() {
  ...
  gh label create "$name" -R "${repo}" --description "$description" --force >/dev/null
}
...
# Platform variables used by bootstrap/runtime configuration.
...
gh variable set CODEXK8S_ENABLE_GITHUB_RUNNER -R "${CODEXK8S_GITHUB_REPO}" --body "${CODEXK8S_ENABLE_GITHUB_RUNNER}"
if [ "${CODEXK8S_ENABLE_GITHUB_RUNNER}" = "true" ]; then
  gh variable set CODEXK8S_STAGING_RUNNER -R "${CODEXK8S_GITHUB_REPO}" --body "${CODEXK8S_STAGING_RUNNER}"
  gh variable set CODEXK8S_RUNNER_NAMESPACE -R "${CODEXK8S_GITHUB_REPO}" --body "${CODEXK8S_RUNNER_NAMESPACE}"
  gh variable set CODEXK8S_RUNNER_SCALE_SET_NAME -R "${CODEXK8S_GITHUB_REPO}" --body "${CODEXK8S_RUNNER_SCALE_SET_NAME}"
  gh variable set CODEXK8S_RUNNER_MIN -R "${CODEXK8S_GITHUB_REPO}" --body "${CODEXK8S_RUNNER_MIN}"
  gh variable set CODEXK8S_RUNNER_MAX -R "${CODEXK8S_GITHUB_REPO}" --body "${CODEXK8S_RUNNER_MAX}"
  gh variable set CODEXK8S_RUNNER_IMAGE -R "${CODEXK8S_GITHUB_REPO}" --body "${CODEXK8S_RUNNER_IMAGE}"
else
  for key in CODEXK8S_STAGING_RUNNER CODEXK8S_RUNNER_NAMESPACE CODEXK8S_RUNNER_SCALE_SET_NAME CODEXK8S_RUNNER_MIN CODEXK8S_RUNNER_MAX CODEXK8S_RUNNER_IMAGE; do
...
gh variable set CODEXK8S_PLATFORM_DEPLOYMENT_REPLICAS -R "${CODEXK8S_GITHUB_REPO}" --body "${CODEXK8S_PLATFORM_DEPLOYMENT_REPLICAS}"
...
# Platform secrets used by bootstrap/runtime configuration.
