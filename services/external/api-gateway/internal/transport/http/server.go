...
func NewServer(initCtx context.Context, cfg ServerConfig, cp *controlplane.Client, auth authService, logger *slog.Logger) (*Server, error) {
	...
	staffGroup.GET("/runs/realtime", staffH.RunsRealtime)
	...
	staffGroup.GET("/system-settings", staffH.ListSystemSettings)
	staffGroup.GET("/system-settings/realtime", staffH.SystemSettingsRealtime)
	staffGroup.GET("/system-settings/:setting_key", staffH.GetSystemSetting)
	...
	staffGroup.GET("/runtime-deploy/tasks/realtime", staffH.RuntimeDeployTasksRealtime)
	...
	staffGroup.GET("/mission-control/workspace", staffH.GetMissionControlWorkspace)
	staffGroup.GET("/mission-control/nodes/:node_kind/:node_public_id", staffH.GetMissionControlNode)
	staffGroup.GET("/mission-control/nodes/:node_kind/:node_public_id/activity", staffH.ListMissionControlNodeActivity)
	staffGroup.POST("/mission-control/launch-preview", staffH.PreviewMissionControlLaunch)
