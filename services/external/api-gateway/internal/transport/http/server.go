...
func NewServer(initCtx context.Context, cfg ServerConfig, cp *controlplane.Client, auth authService, logger *slog.Logger) (*Server, error) {
	...
	e.GET("/api/v1/runs/:run_id/bypass", staffH.GetRunByAccessKey)
	e.GET("/api/v1/runs/:run_id/bypass/events", staffH.ListRunEventsByAccessKey)
	e.GET("/api/v1/runs/:run_id/bypass/logs", staffH.GetRunLogsByAccessKey)
	...
	staffGroup.GET("/runs/:run_id/access-key", staffH.GetRunAccessKeyStatus)
	staffGroup.POST("/runs/:run_id/access-key/regenerate", staffH.RegenerateRunAccessKey)
	staffGroup.POST("/runs/:run_id/access-key/revoke", staffH.RevokeRunAccessKey)
