...
func NewServer(initCtx context.Context, cfg ServerConfig, cp *controlplane.Client, auth authService, logger *slog.Logger) (*Server, error) {
	...
	staffGroup.GET("/agents", staffH.ListAgents)
	staffGroup.GET("/agents/:agent_id", staffH.GetAgent)
	staffGroup.PATCH("/agents/:agent_id/settings", staffH.UpdateAgentSettings)
	staffGroup.GET("/prompt-templates", staffH.ListPromptTemplateKeys)
	staffGroup.GET("/prompt-templates/:template_key/versions", staffH.ListPromptTemplateVersions)
	staffGroup.POST("/prompt-templates/:template_key/versions", staffH.CreatePromptTemplateVersion)
	staffGroup.POST("/prompt-templates/:template_key/versions/:version/activate", staffH.ActivatePromptTemplateVersion)
	staffGroup.POST("/prompt-templates/seeds/sync", staffH.SyncPromptTemplateSeeds)
	staffGroup.POST("/prompt-templates/:template_key/preview", staffH.PreviewPromptTemplate)
	staffGroup.GET("/prompt-templates/:template_key/diff", staffH.DiffPromptTemplateVersions)
	staffGroup.GET("/audit/prompt-templates", staffH.ListPromptTemplateAuditEvents)
	staffGroup.GET("/runs", staffH.ListRuns)
	staffGroup.GET("/runs/jobs", staffH.ListRunJobs)
