...
func (h *staffHandler) ListAgents(c *echo.Context) error {
	return listByLimitResp(c, 200, func(ctx context.Context, principal *controlplanev1.Principal, limit int32) (*controlplanev1.ListAgentsResponse, error) {
		return h.cp.Service().ListAgents(ctx, &controlplanev1.ListAgentsRequest{
			Principal: principal,
			Limit:     limit,
		})
	}, casters.Agents)
...
func (h *staffHandler) GetAgent(c *echo.Context) error {
	return getByPathResp(c, "agent_id", func(ctx context.Context, principal *controlplanev1.Principal, id string) (*controlplanev1.Agent, error) {
		return h.cp.Service().GetAgent(ctx, &controlplanev1.GetAgentRequest{
			Principal: principal,
			AgentId:   id,
		})
	}, casters.Agent)
...
func (h *staffHandler) CreatePromptTemplateVersion(c *echo.Context) error {
	return withPrincipalAndResolved(c, resolvePath("template_key"), func(principal *controlplanev1.Principal, templateKey string) error {
		...
		item, err := h.cp.Service().CreatePromptTemplateVersion(c.Request().Context(), &controlplanev1.CreatePromptTemplateVersionRequest{
			Principal:       principal,
			TemplateKey:     templateKey,
