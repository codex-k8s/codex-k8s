		...
		claims: []runqueuerepo.ClaimedRun{
			{
				...
				ProjectID:     "proj-1",
				RunPayload:    json.RawMessage(`{"repository":{"full_name":"codex-k8s/codex-k8s"},"agent":{"name":"AI Developer"}}`),
				SlotNo:        1,
			},
		},
	}
...
func TestTickLaunchesFullEnvRunWithNamespacePreparation(t *testing.T) {
	...
	payload := json.RawMessage(`{"repository":{"full_name":"codex-k8s/codex-k8s"},"trigger":{"kind":"dev"},"issue":{"number":77},"agent":{"name":"AI Developer"}}`)
	runs := &fakeRunQueue{
		claims: []runqueuerepo.ClaimedRun{
...
func TestTickFinalizesFullEnvRunAndCleansNamespace(t *testing.T) {
	...
	payload := json.RawMessage(`{"repository":{"full_name":"codex-k8s/codex-k8s"},"trigger":{"kind":"dev_revise"},"issue":{"number":10},"agent":{"name":"AI Developer"}}`)
	runs := &fakeRunQueue{
